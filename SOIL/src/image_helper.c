  ...
  for (y = 0; y < resampled_height; ++y) {
    ...
    for (x = 0; x < resampled_width; ++x) {
      ...
      for (c = 0; c < channels; ++c) {
        ...
        value +=
            orig[base_index + width * channels] * (1.0f - samplex) * (sampley);
        value += orig[base_index + width * channels + channels] * (samplex) *
                 (sampley);
...
int mipmap_image(const unsigned char *const orig, int width, int height,
  ...
  for (j = 0; j < mip_height; ++j) {
    for (i = 0; i < mip_width; ++i) {
      for (c = 0; c < channels; ++c) {
        const int index = (j * block_size_y) * width * channels +
...
int scale_image_RGB_to_NTSC_safe(unsigned char *orig, int width, int height,
  ...
  for (i = 0; i < 256; ++i) {
    scale_LUT[i] =
        (unsigned char)((scale_hi - scale_lo) * i / 255.0f + scale_lo);
  }
