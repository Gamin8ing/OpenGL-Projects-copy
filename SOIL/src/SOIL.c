...
unsigned int SOIL_load_OGL_HDR_texture(const char *filename,
                                       int fake_HDR_format, int rescale_to_max,
                                       unsigned int reuse_texture_ID,
                                       unsigned int flags) {
  /*	variables	*/
  unsigned char *img;
...
unsigned int SOIL_load_OGL_texture_from_memory(
  ...
  /*	try to load the image	*/
  img = SOIL_load_image_from_memory(buffer, buffer_length, &width, &height,
                                    &channels, force_channels);
  /*	channels holds the original number of channels, which may have been
   * forced	*/
  if ((force_channels >= 1) && (force_channels <= 4)) {
...
unsigned int SOIL_load_OGL_cubemap(
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img =
        SOIL_load_image(x_neg_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
      channels = force_channels;
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img =
        SOIL_load_image(y_pos_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
      channels = force_channels;
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img =
        SOIL_load_image(y_neg_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
      channels = force_channels;
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img =
        SOIL_load_image(z_pos_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
      channels = force_channels;
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img =
        SOIL_load_image(z_neg_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
      channels = force_channels;
...
unsigned int SOIL_load_OGL_cubemap_from_memory(
  ...
  /*	1st face: try to load the image	*/
  img = SOIL_load_image_from_memory(x_pos_buffer, x_pos_buffer_length, &width,
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img = SOIL_load_image_from_memory(x_neg_buffer, x_neg_buffer_length, &width,
                                      &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img = SOIL_load_image_from_memory(y_pos_buffer, y_pos_buffer_length, &width,
                                      &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img = SOIL_load_image_from_memory(y_neg_buffer, y_neg_buffer_length, &width,
                                      &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img = SOIL_load_image_from_memory(z_pos_buffer, z_pos_buffer_length, &width,
                                      &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
  ...
  if (tex_id != 0) {
    /*	1st face: try to load the image	*/
    img = SOIL_load_image_from_memory(z_neg_buffer, z_neg_buffer_length, &width,
                                      &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
...
unsigned int SOIL_load_OGL_single_cubemap(const char *filename,
  ...
  tex_id = SOIL_create_OGL_single_cubemap(img, width, height, channels,
...
unsigned int SOIL_create_OGL_single_cubemap(const unsigned char *const data,
...
unsigned int SOIL_create_OGL_texture(const unsigned char *const data, int width,
...
unsigned int SOIL_internal_create_OGL_texture(
  ...
  if (flags & SOIL_FLAG_TEXTURE_RECTANGLE) {
    ...
    if (query_tex_rectangle_capability() == SOIL_CAPABILITY_PRESENT) {
      ...
      if (opengl_texture_type == GL_TEXTURE_2D) {
         ...
         */
        flags &= ~(SOIL_FLAG_POWER_OF_TWO | SOIL_FLAG_MIPMAPS |
      ...
      (height > max_supported_size)) /*	2^n for later down-sampling)	*/
  {
    ...
    if ((new_width != width) || (new_height != height)) {
      ...
      unsigned char *resampled =
          (unsigned char *)malloc(channels * new_width * new_height);
      up_scale_image(img, width, height, channels, resampled, new_width,
                     new_height);
  ...
  if ((width > max_supported_size) || (height > max_supported_size)) {
    ...
    mipmap_image(img, width, height, channels, resampled, reduce_block_x,
                 reduce_block_y);
    /*	nuke the old guy, then point it at the new guy	*/
    SOIL_free_image_data(img);
    img = resampled;
  ...
  if (tex_id) {
    ...
    if (DXT_mode == SOIL_CAPABILITY_PRESENT) {
      ...
      if ((channels & 1) == 1) {
        /*	RGB, use DXT1	*/
        DDS_data =
            convert_image_to_DXT1(img, width, height, channels, &DDS_size);
      } else {
        /*	RGBA, use DXT5	*/
        DDS_data =
            convert_image_to_DXT5(img, width, height, channels, &DDS_size);
      }
      if (DDS_data) {
        ...
        glTexImage2D(opengl_texture_target, 0, internal_texture_format, width,
                     height, 0, original_texture_format, GL_UNSIGNED_BYTE, img);
    ...
    if (flags & SOIL_FLAG_MIPMAPS) {
      ...
      unsigned char *resampled =
          (unsigned char *)malloc(channels * MIPwidth * MIPheight);
      while (((1 << MIPlevel) <= width) || ((1 << MIPlevel) <= height)) {
        /*	do this MIPmap level	*/
        mipmap_image(img, width, height, channels, resampled, (1 << MIPlevel),
                     (1 << MIPlevel));
        /*  upload the MIPmaps	*/
        if (DXT_mode == SOIL_CAPABILITY_PRESENT) {
          /*	user wants me to do the DXT conversion!	*/
          ...
          if ((channels & 1) == 1) {
            /*	RGB, use DXT1	*/
            DDS_data = convert_image_to_DXT1(resampled, MIPwidth, MIPheight,
                                             channels, &DDS_size);
          } else {
            /*	RGBA, use DXT5	*/
            DDS_data = convert_image_to_DXT5(resampled, MIPwidth, MIPheight,
                                             channels, &DDS_size);
          }
          if (DDS_data) {
            soilGlCompressedTexImage2D(opengl_texture_target, MIPlevel,
                                       internal_texture_format, MIPwidth,
                                       MIPheight, 0, DDS_size, DDS_data);
            check_for_GL_errors("glCompressedTexImage2D");
            SOIL_free_image_data(DDS_data);
          } else {
            /*	my compression failed, try the OpenGL driver's version	*/
          ...
          /*	user want OpenGL to do all the work!	*/
          glTexImage2D(opengl_texture_target, MIPlevel, internal_texture_format,
                       MIPwidth, MIPheight, 0, original_texture_format,
...
int SOIL_save_screenshot(const char *filename, int image_type, int x, int y,
  ...
  /*	save the image	*/
  save_result =
      SOIL_save_image(filename, image_type, width, height, 3, pixel_data);
...
int SOIL_save_image(const char *filename, int image_type, int width, int height,
  ...
  if (image_type == SOIL_SAVE_TYPE_BMP) {
    save_result =
        stbi_write_bmp(filename, width, height, channels, (void *)data);
  } else if (image_type == SOIL_SAVE_TYPE_TGA) {
    save_result =
        stbi_write_tga(filename, width, height, channels, (void *)data);
...
void SOIL_free_image_data(unsigned char *img_data) { free((void *)img_data); }

const char *SOIL_last_result(void) { return result_string_pointer; }
...
unsigned int SOIL_direct_load_DDS_from_memory(const unsigned char *const buffer,
  ...
  if (uncompressed) {
    ...
    if (query_DXT_capability() != SOIL_CAPABILITY_PRESENT) {
      /*	we can't do it!	*/
      result_string_pointer =
          "Direct upload of S3TC images not supported by the OpenGL driver";
      return 0;
    }
  ...
  for (cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target) {
    if (buffer_index + DDS_full_size <= buffer_length) {
      ...
      if (uncompressed) {
        ...
        glTexImage2D(cf_target, 0, S3TC_type, width, height, 0, S3TC_type,
                     GL_UNSIGNED_BYTE, DDS_data);
      } else {
        soilGlCompressedTexImage2D(cf_target, 0, S3TC_type, width, height, 0,
      ...
      for (i = 1; i <= mipmaps; ++i) {
        ...
        if (uncompressed) {
          ...
          soilGlCompressedTexImage2D(cf_target, i, S3TC_type, w, h, 0, mip_size,
...
unsigned int SOIL_direct_load_DDS(const char *filename,
  ...
  tex_ID = SOIL_direct_load_DDS_from_memory((const unsigned char *const)buffer,
                                            buffer_length, reuse_texture_ID,
