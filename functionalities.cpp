...
GLuint texture[NumBuffers]; // <------------ TEXTURE -> Array of pointers to
...
glm::mat4 model_view;
glm::vec3 unit_z_vector =
    glm::vec3(0, 0, 1); // Assigning a meaningful name to (0,0,1) :-)
glm::vec3 cam_pos = glm::vec3(0.0f, 0.0f, height);
glm::vec3 forward_vector = glm::vec3(
    1, 1, 0); // Forward vector is parallel to the level at all times (No pitch)
...
GLfloat alpha = 0; // Rationale: value that is an angle that keeps being
                   // calculated for every frame in Idle function
...
void setUpPowerPacks() {
  for (size_t i = 0; i < numberOfPowerPacks; i++) {
    ...
    glm::vec3 powerPos =
        glm::vec3(randX, randY, 0.0); // Random Position for power

    gameScene.push_back(
        GameObject('p', 2, powerPos, cam_pos,
                   0.0009)); // look_at -> to the camera position
  }
}
...
void setUpEnemies() {
  for (size_t i = 0; i < numberOfEnemies; i++) {
    ...
    gameScene.push_back(
        GameObject('e', 2, enemyPos, cam_pos,
                   0.0009)); // look_at -> to the camera position
  }
}
...
void init(void) {
  ...
  // This texture is loaded from file. To do this, we use the SOIL (Simple
  // OpenGL Imaging Library) library. When using the SOIL_load_image() function,
  // make sure the you are using correct patrameters, or else, your image will
  // NOT be loaded properly, or will not be loaded at all.
  TextureInfo texturesInfo[NumBuffers];
  texturesInfo[0].imageData =
      SOIL_load_image("grass.png", &texturesInfo[0].width,
                      &texturesInfo[0].height, 0, SOIL_LOAD_RGB);
  texturesInfo[1].imageData =
      SOIL_load_image("wall.jpg", &texturesInfo[1].width,
                      &texturesInfo[1].height, 0, SOIL_LOAD_RGB);
  texturesInfo[2].imageData =
      SOIL_load_image("energy.jpg", &texturesInfo[2].width,
                      &texturesInfo[2].height, 0, SOIL_LOAD_RGB);
  texturesInfo[3].imageData =
      SOIL_load_image("fire.jpg", &texturesInfo[3].width,
                      &texturesInfo[3].height, 0, SOIL_LOAD_RGB);
  texturesInfo[4].imageData =
      SOIL_load_image("demon.jpg", &texturesInfo[4].width,
                      &texturesInfo[4].height, 0, SOIL_LOAD_RGB);
  texturesInfo[5].imageData =
      SOIL_load_image("ammo.png", &texturesInfo[5].width,
                      &texturesInfo[5].height, 0, SOIL_LOAD_RGB);
  texturesInfo[6].imageData =
      SOIL_load_image("skyNight.jpg", &texturesInfo[6].width,
                      &texturesInfo[6].height, 0, SOIL_LOAD_RGB);
  ...
  for (size_t i = 0; i < NumBuffers; i++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texturesInfo[i].width,
                 texturesInfo[i].height, 0, GL_RGB, GL_UNSIGNED_BYTE,
                 texturesInfo[i].imageData);
...
void drawCube(float scale, GLuint *selectedTexture) {
  ...
  // Select the second texture (apple.png) when drawing the second geometry
  // (cube)
...
// Renders level
void draw_level() {
  // Select the first texture (grass.png) when drawing the first geometry
  // (floor)
  ...
  for (int i = 0; i < Num_Obstacles; i++) {
    model_view = glm::translate(
        model_view, glm::vec3(obstacle_data[i][0], obstacle_data[i][1], 0.0));
    glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
...
// Rationale: the code bellow will draw all the objects in the scene that are
// listed in the gameScene vector
...
void updateScene() {
  ...
  for (size_t i = 0; i < gameScene.size(); i++) {
    model_view = glm::translate(
        model_view,
        gameScene[i].getLocation()); // move the object to its own location x
                                     // and y (z will be 0)
    glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
      ...
      for (size_t j = 0; j < gameScene.size() && !playerBulletCollides; j++) {
        if (i != j && gameScene[j].checkIsEnemy()) {
          if (gameScene[i].isCollidingWith(gameScene[j].getLocation())) {
            ...
            std::cout << "----------> PLAYER [ Killed Enemy ]: "
                      << playerInfo.enemyKills << std::endl;
          }
        }
      }

      if (!playerBulletCollides)
        gameScene[i].incrementPosition(); // increment bullet position
    ...
    } else if (gameScene[i].checkIsEnemyBullet()) // <------- BULLET ENEMY
    {
      ...
      if (gameScene[i].isCollidingWithPlayer()) {
        ...
        std::cout << "----------> PLAYER [ Life ]: " << playerInfo.life
                  << " (Hit By Demon's Bullet)" << std::endl;
      }
    ...
    } else // <------- ENEMY or POWER PACK
    {
      ...
      if (gameScene[i].isCollidingWithPlayer()) {
        ...
        if (gameScene[i].checkIsPowerPack()) // power pack + player
        {
          ...
          std::cout << "----------> PLAYER [ Life ]: " << playerInfo.life
                    << " (Got Power Pack)" << std::endl;
        } else // enemy + player
        {
          playerInfo.life--;
          playerInfo.enemyTouch++;
          std::cout << "----------> PLAYER [ Life ]: " << playerInfo.life
                    << " (Touched By a Demon)" << std::endl;
        }
      }
    }
  }
...
bool checkForGameEnd() { return playerInfo.life <= 0; }
...
void endGameResults() {
  ...
  std::cout << "|                    OVER VIEW                     |"
            << std::endl;
  std::cout << "| Player Got: " << playerInfo.powerPacks << " Power Packs"
            << std::endl;
  std::cout << "| Player Killed: " << playerInfo.enemyKills << " Enemies"
            << std::endl;
  std::cout << "| Player Got Touched: " << playerInfo.enemyTouch
            << " times by Enemies" << std::endl;
...
void updateCameraState() {
  ...
  glm::mat4 proj_matrix =
      glm::frustum(-0.01f, +0.01f, -0.01f, +0.01f, 0.01f, 1000.0f);
...
void keyboard(unsigned char key, int x, int y) {
  ...
  if (key == 'w') {
    // Moving camera along forward vector. To be more realistic, we use X=V.T
    // equation in physics
    cam_pos += forward_vector * travel_speed * ((float)deltaTime) / 1000.0f;
  }
  if (key == 's') {
    // Moving camera along backward (negative forward) vector. To be more
    // realistic, we use X=V.T equation in physics
    cam_pos -= forward_vector * travel_speed * ((float)deltaTime) / 1000.0f;
  }
...
void generateProjectile(GLchar objType, glm::vec3 dir_vector) {
  ...
  gameScene.push_back(GameObject(objType, collisionRadius, bulletStartLocation,
                                 dir_vector, bullSpeed));
}
...
void mouse(int x, int y) {
  ...
  forward_vector =
      glm::rotate(forward_vector, -delta_x * mouse_sensitivity, unit_z_vector);
  looking_dir_vector = glm::rotate(looking_dir_vector,
                                   -delta_x * mouse_sensitivity, unit_z_vector);
  side_vector =
      glm::rotate(side_vector, -delta_x * mouse_sensitivity, unit_z_vector);
  up_vector =
      glm::rotate(up_vector, -delta_x * mouse_sensitivity, unit_z_vector);
  x0 = x;

  // The following vectors must get updated during a pitch movement
  int delta_y = y - y_0;
  glm::vec3 tmp_up_vec =
      glm::rotate(up_vector, delta_y * mouse_sensitivity, side_vector);
  glm::vec3 tmp_looking_dir =
      glm::rotate(looking_dir_vector, delta_y * mouse_sensitivity, side_vector);

  // The dot product is used to prevent the user from over-pitch (pitching 360
  // degrees) The dot product is equal to cos(theta), where theta is the angle
  // between looking_dir and forward vector
  GLfloat dot_product = glm::dot(tmp_looking_dir, forward_vector);

  // If the angle between looking_dir and forward vector is between (-90 and 90)
  // degress
  if (dot_product > 0) {
    up_vector =
        glm::rotate(up_vector, delta_y * mouse_sensitivity, side_vector);
    looking_dir_vector = glm::rotate(looking_dir_vector,
                                     delta_y * mouse_sensitivity, side_vector);
  }
  y_0 = y;
}
...
void idle() {
  ...
  // Rationale: the code bellow is being used to calculate for each frame the
  // new value for the angle alpha,
  //            in order to keep rotating the objects that the user adds to the
  //            scene
  GLfloat omega =
      10.0f; // let omega  be the angular speed for Global Axes rotation
  GLfloat rotatetimeSinceStart =
      glutGet(GLUT_ELAPSED_TIME) / 1000.f; // Retreives the time in seconds
  alpha =
      omega *
      rotatetimeSinceStart; // We use alpha = omega * time to calculate the
                            // amount of rotation of the geometry in this frame
  // ---------
